...
// Note: We can't set standardFontDataUrl directly as it's not in the type definitions
  ...
  try {
    ...
    for (let i = 1; i <= maxPages; i++) {
      ...
      const textContent2 = await page.getTextContent({
        // normalizeWhitespace option removed as it's not in the type definitions
